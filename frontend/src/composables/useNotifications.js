...
class NotificationManager {
  ...
  init() {
    // 请求桌面通知权限
    if (typeof window !== 'undefined' && 'Notification' in window && config.enableDesktop) {
  ...
  registerToast(component) {
    ...
    if (component) {
      notificationQueue.pending.splice(0).forEach(item => component.addToast(item))
    }
  }
  ...
  addNotification(notification) {
    ...
    if (toastComponent.value) {
      ...
      notificationQueue.pending.push(fullNotification)
    }
        ...
        actions: [
          {
            ...
            type: 'primary',
            handler: () => {
              resolve(generation === getAuthGeneration())
            }
          },
          {
  ...
  // 清除所有通知
  clearAll() {
    notificationQueue.pending.splice(0).forEach(item => item.onClose?.(item))
    notifications.value = []
