<template>
  <div class="file-upload-container">
    ...
    <div 
      ...
      role="button" tabindex="0" aria-label="选择上传文件"
    >
      <input 
        ref="fileInput"
        type="file" aria-label="上传文件"
        multiple 
      ...
      <div class="file-items">
        ...
        >
          ...
          <div class="file-actions">
            ...
            <div v-else-if="file.status === 'success'" class="success-indicator" role="status" aria-label="上传成功">
...
import { uploadSingleFile } from '@/services/api.js'
...
const addFiles = (newFiles) => {
  ...
  if (files.value.length + newFiles.length > props.maxFiles) {
    notifications.info(`最多只能选择 ${props.maxFiles} 个文件`)
    return
  }
  ...
  const validFiles = newFiles.filter(file => {
    ...
    if (!isValidType) {
      notifications.info(`不支持的文件类型: ${file.name}`)
      return false
    }
    ...
    if (file.size > maxSize) {
      notifications.info(`文件过大: ${file.name} (最大 ${isImage ? '10MB' : '50MB'})`)
      return false
    }
...
const uploadSingleFileItem = async (fileItem) => {
  ...
  fileItem.progress = 0
  try {
    const result = await uploadSingleFile(fileItem.file)
    if (result.success === false) throw new Error(result.error || '上传失败')
    fileItem.status = 'success'
    fileItem.progress = 100
    fileItem.result = result.data || result
  } catch (error) {
...
const uploadMultipleFileItems = async (fileItems) => {
  // Each file retains its own success or failure, without simulated upload progress.
  for (const fileItem of fileItems) await uploadSingleFileItem(fileItem)
}
