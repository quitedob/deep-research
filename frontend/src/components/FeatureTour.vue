    ...
    <div class="tour-navigation">
      ...
      <div class="tour-dots">
        <span v-for="(feature, index) in features" :key="index"
              ...
              @click="goToStep(index)" @keydown.enter="goToStep(index)" @keydown.space.prevent="goToStep(index)" role="button" tabindex="0" :aria-label="`导览第 ${index + 1} 步`"></span>
      </div>

      <button @click="currentStep === totalSteps - 1 ? completeTour() : nextStep()" class="btn btn-primary">
...
.chart-bar {
  width: 30px;
  background: linear-gradient(to top, #0369a1, #0f766e);
