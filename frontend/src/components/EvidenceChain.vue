  ...
  <div class="evidence-chain">
    ...
    <div class="evidence-stats" v-if="stats">
      ...
      <div class="stat-item">
        ...
        <span class="stat-value">{{ stats.avg_relevance_score == null ? '未评估' : stats.avg_relevance_score.toFixed(2) }}</span>
      </div>
    </div>
        ...
        <div class="evidence-info">
          <div class="evidence-scores">
            <span class="score-item">
              <i class="icon-star"></i>
              相关性: {{ evidence.relevance_score == null ? '未评估' : evidence.relevance_score.toFixed(2) }}
            </span>
            <span class="score-item">
              <i class="icon-target"></i>
              置信度: {{ evidence.confidence_score == null ? '未评估' : evidence.confidence_score.toFixed(2) }}
            </span>
...
const toggleEvidenceVerified = async (evidenceId, verified) => {
  ...
  try {
    ...
    // 暂时使用标记使用状态的API作为替代
    await evidenceAPI.verifyEvidence(evidenceId, verified)
    const evidence = evidenceList.value.find(e => e.id === evidenceId)
    if (evidence) {
...
const loadEvidence = async () => {
  ...
  try {
    ...
    stats.value = {
      ...
      avg_relevance_score: null // 需要从单独的统计API获取
    }

    // 获取统计信息
    try {
      const statsResult = await evidenceAPI.getEvidenceStats()
      stats.value.avg_relevance_score = statsResult.avg_relevance_score ?? null
    } catch (error) {
    ...
    notifications.error(`加载证据链失败：${error.message}`)
    evidenceList.value = []
    stats.value = null
  } finally {
    loading.value = false
