<template>
  <div class="input-box-container">
    <div class="main-input-wrapper">
      <div class="text-area-wrapper">
        <textarea
            v-model="inputText"
            @keydown="handleMessageKeydown($event, sendMessage)"
            aria-label="消息"
    ...
    <div class="attachments-bar">
      <div class="plus-menu-container" ref="plusMenuContainerRef">
        <button class="attach-btn plus-btn" title="添加" aria-label="添加" @click="toggleAttachmentMenu">
          <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><line x1="12" y1="5" x2="12" y2="19"></line><line x1="5" y1="12" x2="19" y2="12"></line></svg>
        </button>
        <div v-if="isAttachmentMenuOpen" class="attachment-menu-popup">
          <ul>
            <li @click="handleMenuAction('upload_files')" @keydown.enter="handleMenuAction('upload_files')" @keydown.space.prevent="handleMenuAction('upload_files')" role="button" tabindex="0">
      ...
      <button
        ...
        :class="{ 'active': isDeepThinkMode }" :aria-pressed="isDeepThinkMode" aria-label="深度思考"
      ...
      <button
        ...
        :class="{ 'active': isWebSearchMode }" :aria-pressed="isWebSearchMode" aria-label="联网搜索"
      ...
      <button
        ...
        :class="{ 'active': isDeepResearchMode }" :aria-pressed="isDeepResearchMode" aria-label="深度研究"
      ...
      <button class="attach-btn" title="画布" aria-label="画布">
      ...
      <button class="attach-btn mic-btn" title="语音输入" aria-label="语音输入">
  ...
  <div v-if="showFileUpload" class="file-upload-overlay">
    <div class="file-upload-modal" v-dialog="() => showFileUpload = false">
      <div class="modal-header">
        <h3>文件上传</h3>
        <button aria-label="关闭文件上传" @click="showFileUpload = false" class="close-btn">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <line x1="18" y1="6" x2="6" y2="18"></line>
            <line x1="6" y1="6" x2="18" y2="18"></line>
          </svg>
...
import { handleMessageKeydown } from '@/utils/input.js';
...
const sendMessage = () => {
  ...
  if (!text || props.disabled) return; // 不发送空消息
...
const handleMenuAction = (action) => {
  ...
  if (action === 'upload_files') {
    ...
    // TODO: 实现云盘添加功能
    notifications.info('云盘添加功能正在开发中...');
  } else if (action === 'import_code') {
    // TODO: 实现代码导入功能
    notifications.info('代码导入功能正在开发中...');
  }
};
...
const handleFileUploadSuccess = (data) => {
  ...
  if (data.files.length > 0) {
    const processedContent = data.files
      .map(file => `[文件: ${file.name}] ${file.result?.processed_content || file.result?.content_preview || file.result?.description || '已上传'}`)
      .join('\n');
...
const handleFileUploadError = (error) => {
  ...
  notifications.error(`文件上传失败: ${error.message || error}`);
};
