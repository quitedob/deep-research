  ...
  <div class="settings-section">
    ...
    <div class="setting-item">
      <div class="setting-label">
        <label>清除对话历史</label>
        <p class="setting-description">删除当前账号在服务器上保存的全部对话记录</p>
      </div>
    ...
    <div class="setting-item">
      ...
      <div class="setting-control">
        <select v-model="autoCleanDays" @change="updateAutoClean" class="setting-select" aria-label="自动清理">
...
const clearHistory = async () => {
  ...
  if (!await notifications.confirm('确定要清除所有对话历史吗？此操作不可恢复！')) return;
  if (generation !== getAuthGeneration()) return;
  
  try {
    if (!await chatStore.deleteAllHistories(generation)) return;
    if (generation !== getAuthGeneration()) return;
    localStorage.removeItem('chat_history');
    notifications.info('对话历史已清除');
  } catch (error) {
    if (generation !== getAuthGeneration()) return;
    console.error('清除失败:', error);
    notifications.error('清除失败，请重试');
  }
};
