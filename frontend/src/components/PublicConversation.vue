...
const formatMessageContent = (content) => md.render(content || '');
...
const loadPublicConversation = async () => {
  try {
    ...
    const data = await request(`/api/public/conversation/${shareId.value}`, { auth: false });
    conversationData.value = data;

  } catch (err) {
    console.error('加载公开对话失败:', err);
    error.value = err.message || '网络连接错误，请稍后重试';
  } finally {
    loading.value = false;
  }
};
...
const shareAgain = async () => {
  try {
    ...
    notifications.error('复制失败，请手动复制链接');
  }
};
