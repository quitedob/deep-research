...
from sqlalchemy import Integer, String, DateTime, ForeignKey, Text, Float, Index
...
class DocumentProcessingJob(Base):
    ...
    result: Mapped[Optional[str]] = mapped_column(JSONB, nullable=True)  # 处理结果元数据
...
class Document(Base):
    ...
    # 证据链支持
    source_url: Mapped[Optional[str]] = mapped_column(String(1024), nullable=True, index=True)  # 来源URL
    ...
    content_hash: Mapped[Optional[str]] = mapped_column(String(64), nullable=True, index=True)  # 内容哈希（去重）
    ...
    # 元数据
    metadata: Mapped[Optional[dict]] = mapped_column(JSONB, nullable=True)  # 文档元数据
    tags: Mapped[Optional[List[str]]] = mapped_column(JSONB, nullable=True)  # 标签列表
    ...
    # 索引优化
    __table_args__ = (
        Index('ix_documents_user_source', 'user_id', 'source_url'),
...
class Chunk(Base):
    ...
    # 元数据
    metadata: Mapped[Optional[dict]] = mapped_column(JSONB, nullable=True)  # 分块元数据

    # 时间戳
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)

    document: Mapped[Document] = relationship(back_populates="chunks")
    embedding: Mapped[Optional[Embedding]] = relationship(back_populates="chunk", cascade="all, delete-orphan")
...
class Embedding(Base):
    ...
    # pgvector 向量存储 (1536维 for OpenAI embeddings, 可调整)
    vector: Mapped[Optional[List[float]]] = mapped_column(Vector(1536), nullable=True)
    ...
    # 创建向量索引以提升检索性能
    __table_args__ = (
        Index('ix_embeddings_vector_cosine', 'vector', postgresql_using='ivfflat', postgresql_ops={'vector': 'vector_cosine_ops'}),
        Index('ix_embeddings_vector_l2', 'vector', postgresql_using='ivfflat', postgresql_ops={'vector': 'vector_l2_ops'}),
    )
...
class Evidence(Base):
    ...
    # 证据来源
    source_type: Mapped[str] = mapped_column(String(32), index=True)  # 'document', 'web', 'api', 'search', 'database'
    ...
    __table_args__ = (
        ...
        Index('ix_evidence_source_type_url', 'source_type', 'source_url'),
