...
@api_router.post("/llm/chat", response_model=ChatResp)
...
async def start_research(req: ResearchReq):
    """最小可用研究端点：调用简单 LLM 生成报告并保存，便于前端轮询/订阅。"""
    session_id = await store.ensure_session(req.session_id)
    prompt = f"请根据以下研究主题，生成结构化研究大纲并给出关键参考方向：\n\n{req.query}\n\n输出包含：摘要、研究问题、方法、数据来源、相关工作、初步结论。"
    result = await _router.chat(
        task="research",
        size="large",
        messages=[LLMMessage(role="user", content=prompt)],
        temperature=0.7,
    )
    await store.set_research_report(session_id, req.query, sanitize_model_output(result["content"]))
...
async def research_stream(session_id: str):
    ...
    async def _gen():
        try:
            yield b"data: {\"phase\":\"planning\",\"message\":\"生成研究计划...\"}\n\n"
            await asyncio.sleep(0.05)
            yield b"data: {\"phase\":\"collecting\",\"message\":\"检索与收集资料...\"}\n\n"
            await asyncio.sleep(0.05)
            yield b"data: {\"phase\":\"synthesizing\",\"message\":\"撰写报告...\"}\n\n"
            await asyncio.sleep(0.05)
