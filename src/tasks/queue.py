...
class TaskQueue:
    ...
    async def enqueue(
        ...
        **kwargs
        ...
        await self.redis.lpush(f"task_queue:{priority.value}", task_data)
    ...
    async def dequeue(self, priority: TaskPriority = TaskPriority.NORMAL) -> Optional[Task]:
        """从队列中取出任务"""
        await self.connect()
        
        # 按优先级从高到低尝试获取任务
        for p in range(TaskPriority.URGENT.value, TaskPriority.LOW.value - 1, -1):
            task_data = await self.redis.rpop(f"task_queue:{p}")
            if task_data:
    ...
    async def retry_task(self, task_id: str) -> bool:
        ...
        # 重新加入队列
        task_data = task.model_dump_json()
        await self.redis.lpush(f"task_queue:{task.priority.value}", task_data)
        ...
        try:
            ...
            if asyncio.iscoroutinefunction(handler):
                ...
                result = await loop.run_in_executor(
                    None, handler, *task.args, **task.kwargs
                )
...
async def enqueue_task(
    ...
    **kwargs
    ...
    return await queue.enqueue(task_name, *args, priority=priority, **kwargs)
