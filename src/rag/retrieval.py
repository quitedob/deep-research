...
class RetrievalService:
    ...
    async def search_documents(
        ...
        """搜索文档 - 优先使用 pgvector，回退到内存存储"""
        try:
            # 尝试使用 pgvector 存储
            from .pgvector_store import get_pgvector_store
            pgvector_store = get_pgvector_store()
            
            if strategy == RetrievalStrategy.VECTOR_ONLY:
                # 使用带证据链的搜索
                search_results = await pgvector_store.search_with_evidence(
                    query, top_k, conversation_id, research_session_id
                )
                ...
                logger.info(f"pgvector 搜索返回 {len(retrieval_results)} 个结果")
                return retrieval_results
            
        except Exception as e:
            logger.warning(f"pgvector 搜索失败，回退到内存存储: {e}")
