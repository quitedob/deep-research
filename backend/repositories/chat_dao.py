...
class ChatDAO(BaseDAO):
    ...
    async def branch_session(self, session_id: str, before_message_id: int, user_id: str):
        """Copy the prefix before an owned user turn, preserving the original chat."""
        return await self.fetch_one(
            """WITH prefix AS (
                   SELECT * FROM chat_messages WHERE session_id=$1 AND id<$2
               ), branch AS (
                   INSERT INTO chat_sessions
                       (id,user_id,title,llm_provider,model_name,system_prompt,message_count,created_at,updated_at)
                   SELECT $4,s.user_id,s.title,s.llm_provider,s.model_name,s.system_prompt,
                          (SELECT COUNT(*) FROM prefix),$5,$5
                   FROM chat_sessions s JOIN chat_messages pivot ON pivot.session_id=s.id
                   WHERE s.id=$1 AND s.user_id=$3 AND pivot.id=$2 AND pivot.role='user'
                   RETURNING *
               ), copied AS (
                   INSERT INTO chat_messages(session_id,role,content,model_name,tokens_used,metadata,created_at)
                   SELECT b.id,p.role,p.content,p.model_name,p.tokens_used,p.metadata,p.created_at
                   FROM prefix p CROSS JOIN branch b ORDER BY p.id
                   RETURNING id
               ) SELECT * FROM branch""",
            (session_id, before_message_id, user_id, str(uuid.uuid4()), utc_now()),
        )
    ...
    async def add_message(
        ...
        query = """
            ...
            UPDATE chat_sessions
            SET message_count = message_count + 1, updated_at = $7
            WHERE id = $1
            RETURNING (SELECT id FROM new_message) AS id
        """
        result = await self.fetch_one(
            query,
            (session_id, role, content, model_name, tokens_used,
             json.dumps(metadata) if metadata is not None else None, utc_now()),
        )
        if result is None:
    ...
    async def clear_session_messages(self, session_id: str) -> bool:
        ...
        result = await self.execute_query(
            """WITH deleted AS (
                   DELETE FROM chat_messages WHERE session_id = $2
               )
