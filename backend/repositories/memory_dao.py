...
class MemoryDAO(BaseDAO):
    ...
    async def get_fact_by_embedding_id(self, embedding_id: str, user_id: str) -> Optional[Dict[str, Any]]:
        """Resolve a vector-store identifier only within the owner's facts."""
        return await self.fetch_one(
            "SELECT * FROM user_facts WHERE embedding_id = $1 AND user_id = $2",
    ...
    async def update_fact_validity(
        ...
        """
        try:
            await self.execute_query(query, (validity_score, utc_now(), fact_id))
            return True
        except Exception as e:
    ...
    async def delete_fact(self, fact_id: str, user_id: str) -> bool:
        """删除事实"""
        query = "DELETE FROM user_facts WHERE id = $1 AND user_id = $2"
        try:
            result = await self.execute_query(query, (fact_id, user_id))
