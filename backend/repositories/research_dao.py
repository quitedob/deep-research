...
def _utc_timestamp(value: Optional[datetime] = None) -> datetime:
    # Existing PostgreSQL TIMESTAMP columns store naive UTC in every DAO.
...
class ResearchDAO(BaseDAO):
    ...
    async def update_session_status(
        ...
        await self.execute_query(
            query,
            (session_id, status, _utc_timestamp(ended_at) if ended_at else None, _utc_timestamp())
        )
    ...
    async def add_research_finding(
        ...
        if result is None:
            raise RuntimeError("Insert returned no persisted record")
        return result["id"]
    ...
    async def get_research_findings(
        ...
        WHERE session_id = $1
        AND (relevance_score >= $2 OR ($2 <= 0 AND relevance_score IS NULL))
        """
    ...
    async def search_research_content(
        ...
        """Search all owned content or a specific session, with a shared outer scope."""
        if not (session_id or user_id) or not query.strip():
            return []
        if not 1 <= limit <= 1000:
        ...
        WITH searchable AS (
            ...
            SELECT 'citation' AS content_type, id, session_id, title AS source_type,
                   title || ' ' || COALESCE(array_to_string(authors, ' '), '') AS text,
                   NULL::double precision AS relevance_score, created_at
            FROM research_citations
            UNION ALL
            SELECT 'memory' AS content_type, id, session_id, message_role AS source_type,
                   message_content AS text, NULL::double precision AS relevance_score, created_at
            FROM research_memory
        )
        SELECT content.*
        FROM searchable AS content
        JOIN research_sessions AS session ON session.id = content.session_id
        WHERE ($2::text IS NULL OR content.session_id = $2)
          AND ($3::text IS NULL OR session.user_id = $3)
          AND to_tsvector('english', content.text) @@ plainto_tsquery('english', $1)
        ORDER BY relevance_score DESC NULLS LAST, created_at DESC
        LIMIT $4
        """
        return await self.fetch_all(search_sql, (query, session_id, user_id, limit))
    ...
    async def get_research_statistics(
        ...
        return {
            ...
            "timestamp": _utc_timestamp().isoformat()
        }
    ...
    async def delete_research_session(self, session_id: str) -> None:
        ...
        Args:
            session_id: 会话ID
        """
        # The foreign keys cascade within this single atomic statement.
    ...
    async def export_session_data(self, session_id: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "citations": citations,
            "memory": memory,
            "exported_at": _utc_timestamp().isoformat()
        }