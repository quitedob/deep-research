...
class AgentScopeLLMAdapter(ChatModelBase):
    ...
    async def __call__(
        ...
        **kwargs
        ...
        if any(isinstance(message, Msg) for message in messages):
            formatter = (OllamaChatFormatter() if self.base_llm.get_provider_name() == "ollama"
    ...
    def _convert_messages_to_base_format(self, messages: List[Union[Msg, Dict]]) -> List[Dict[str, str]]:
        ...
        Returns:
            BaseLLM格式的消息列表
        """
        # Formatters already produced provider messages. Preserve tool_call_id,
        # tool_calls and structured image blocks for the next reasoning step.
...
class DualLLMManager:
    ...
    def __init__(
        ...
        primary_model_name: str = "deepseek-flash",
        ...
        for msg in messages:
            ...
            if isinstance(content, list):
                for block in content:
                    if isinstance(block, dict) and block.get('type') in ('image', 'image_url', 'video'):
                        return True
