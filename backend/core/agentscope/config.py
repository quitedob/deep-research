...
class MultimodalLLMConfig(BaseModel):
    """多模态LLM配置模型"""
    provider: LLMProvider = LLMProvider.DEEPSEEK
    model_name: str = Field(default="deepseek-flash", description="多模态模型名称")
    host: str = Field(default="https://api.deepseek.com", description="多模态API地址")
...
class ConfigManager:
    ...
    def load_config(self) -> AgentScopeConfig:
        ...
        if os.path.exists(self.config_file):
            ...
            # Loading runtime defaults must not create a credential-bearing file.
    ...
    def save_config(self) -> None:
        ...
        with open(self.config_file, 'w', encoding='utf-8') as f:
            json.dump(self._without_secrets(self._config.model_dump(mode="json")), f, indent=2, ensure_ascii=False)

    @staticmethod
    def _without_secrets(value):
        if isinstance(value, dict):
            return {key: ConfigManager._without_secrets(item) for key, item in value.items()
                    if key.lower() not in ("api_key", "token", "access_token", "refresh_token", "auth_token", "password", "secret", "secret_key", "authorization")
                    and not key.lower().endswith(("_api_key", "_password", "_secret"))}
        if isinstance(value, list):
            return [ConfigManager._without_secrets(item) for item in value]
        return value
    ...
    def _create_default_config(self) -> AgentScopeConfig:
        ...
        return AgentScopeConfig(
            ...
            multimodal_llm=MultimodalLLMConfig(
                provider=LLMProvider.DEEPSEEK,
                model_name="deepseek-flash",
                host=os.getenv("DEEPSEEK_BASE_URL", "https://api.deepseek.com")
            )
        )
