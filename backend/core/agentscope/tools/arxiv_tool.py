...
class ArXivTool:
    ...
    async def _search_arxiv(self, query: str, max_results: int = 10,
        ...
        try:
            # 构建查询参数
            params = {
                'search_query': query if re.search(r'\b(?:all|ti|au|abs|co|jr|cat|rn|id|submittedDate|lastUpdatedDate):', query) else f'all:{query}',
                'start': 0,
                'max_results': max_results,
                'sortBy': sort_by,
            ...
            for entry in root.findall('atom:entry', ns):
                ...
                if paper.get('pdf_url'):
                    paper['links']['pdf'] = {'href': paper['pdf_url']}
                paper['updated'] = entry.findtext('atom:updated', default=published, namespaces=ns)
