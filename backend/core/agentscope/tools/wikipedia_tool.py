...
class WikipediaTool:
    ...
    async def _fetch_json(self, request, timeout):
        async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=timeout)) as client:
            async with client.get(request.full_url, headers=dict(request.header_items())) as response:
    ...
    async def search_wikipedia(
        ...
        try:
            ...
            for attempt in range(max_retries):
                try:
                    data = await self._fetch_json(req, 10)
                    break
                except aiohttp.ClientResponseError as e:
                    if e.status in (429, 500, 502, 503, 504) and attempt < max_retries - 1:
                        logger.info(f"[WARN] Wikipedia 403 error, retrying in {retry_delay}s (attempt {attempt + 1}/{max_retries})")
    ...
    async def get_page_content(self, page_title: str, lang: str = "zh") -> Optional[str]:
        ...
        try:
            ...
            data = await self._fetch_json(req, 15)
            ...
            logger.info(f"获取Wikipedia内容失败: {str(e)}")
            return None
