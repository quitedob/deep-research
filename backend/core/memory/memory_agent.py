...
class MemoryAgent:
    ...
    def __init__(
        ...
        self.extraction_model = extraction_model or self.settings.generation_model
        self.generator = LLMFactory.create_llm(provider="deepseek", model=self.extraction_model)
...
请提取值得记忆的事实（JSON格式）："""
    ...
    def _validate_fact(self, fact: Dict[str, Any]) -> bool:
        ...
        # Only retain a score actually returned by the extraction model.
        try:
            score = float(fact["validity_score"])
            if not 0.0 <= score <= 1.0:
                return False
            fact["validity_score"] = score
        ...
        if not isinstance(content, str):
            return False
        if len(content) < self.settings.min_fact_length or len(content) > self.settings.max_fact_length:
    ...
    async def batch_process_unprocessed_messages(
        ...
        try:
            ...
            messages = await self.memory_dao.get_unprocessed_messages(
                ...
                limit=max_messages or self.settings.batch_size
            )
