...
from backend.core.security.passwords import hash_password, verify_password, is_legacy_password_hash

from backend.repositories.user_dao import UserDAO
from backend.schemas.user import UserRegister, UserLogin, UserUpdate, UserPreferences
...
class UserService:
    ...
    async def register(self, user_data: UserRegister) -> Optional[Dict[str, Any]]:
        ...
        # 创建用户
        password_hash = await asyncio.to_thread(self.hash_password, user_data.password)
    ...
    async def login(self, login_data: UserLogin) -> Optional[Dict[str, Any]]:
        ...
        # 验证密码
        if not await asyncio.to_thread(self.verify_password, login_data.password, user['password_hash']):
            raise ValueError("用户名或密码错误")
        ...
        if is_legacy_password_hash(user['password_hash']):
            upgraded_hash = await asyncio.to_thread(self.hash_password, login_data.password)
            await self.user_dao.update_password_hash(user['id'], upgraded_hash)
    ...
    async def get_current_user(self, token: str) -> Optional[Dict[str, Any]]:
        ...
        """
        # 验证Token并检查黑名单
        is_valid, payload = await self.jwt_manager.verify_and_check_blacklist(token, expected_type="access")
        ...
        return user if user and user.get("is_active", False) else None
    ...
    async def refresh_token(self, refresh_token: str) -> Optional[Dict[str, Any]]:
        ...
        if not payload or payload.get("type") != "refresh":
            return None
        user = await self.user_dao.get_user_by_id(payload["sub"])
