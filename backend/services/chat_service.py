...
from backend.core.llm.base_llm import BaseLLM, APIError
...
class ChatService:
    ...
    def __init__(self):
        ...
        try:
            if not get_memory_config().enabled:
                raise RuntimeError("Memory disabled by configuration")
    ...
    def _get_llm_instance(self, provider: str, model_name: str) -> BaseLLM:
        ...
        if provider == "agentscope":
            provider = "deepseek"
            model_name = get_config().get_provider_config(provider).default_model
        ...
        if key not in self._llm_instances:
            ...
            self._llm_instances[key] = LLMFactory.create_llm(provider=provider, model=model_name)
        
        return self._llm_instances[key]
    
    async def prepare_chat(self, chat_request: ChatRequest) -> Dict[str, Any]:
        session = await self.chat_dao.get_session(chat_request.session_id)
        if not session:
            raise ValueError("会话不存在")
        if session["llm_provider"] == "agentscope":
            session = dict(session, llm_provider="deepseek",
                           model_name=get_config().deepseek.default_model)
        self._get_llm_instance(session["llm_provider"], session["model_name"])
        return session
    ...
    async def chat(
        ...
        """
        # 获取会话信息
        session = await self.prepare_chat(chat_request)
        ...
        if self.memory_enabled and self.memory_manager:
            try:
                relevant_memories = await self.memory_manager.retrieve_memories(
                    ...
                    top_k=get_memory_config().retrieval_top_k,
                    use_hyde=get_memory_config().hyde_enabled
                )
    ...
    async def chat_stream(
        ...
        # 保存完整的助手回复
        message_id = await self.chat_dao.add_message(
            session_id=chat_request.session_id,
            role="assistant",
            content=full_content,
            model_name=session['model_name']
        )
        message = {"id": message_id, "role": "assistant", "content": full_content,
                   "model_name": session["model_name"]}
    ...
    async def web_search_chat(
        ...
        if not result['success']:
            raise APIError("Web search failed")
