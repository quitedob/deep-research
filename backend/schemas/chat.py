...
class ChatMessage(BaseModel):
    """对话消息"""
    id: Optional[int] = Field(None, description="持久化消息ID")
...
class ChatSessionCreate(BaseModel):
    ...
    model_name: str = Field("deepseek-flash", description="模型名称")
...
class ChatBranchRequest(BaseModel):
    before_message_id: int = Field(..., gt=0, description="创建分支时排除的用户消息ID")
