...
from backend.schemas.chat import (
    ...
    ChatRequest, ChatResponse, ChatMessage, ModelListResponse, ChatBranchRequest
)
from backend.services.chat_service import ChatService
from backend.api.user import get_current_user_id
from backend.core.security.quota import enforce_request_quota
from backend.core.llm.base_llm import LLMError, ConfigurationError
...
async def create_session(
    ...
    try:
        ...
        raise HTTPException(status_code=502, detail="模型服务暂时不可用，请重试") from exc
    except Exception as e:
        logger.error(f"创建会话失败: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="请求处理失败，请重试")


@router.post("/sessions/{session_id}/branch", response_model=ChatSessionResponse)
async def branch_session(session_id: str, request: ChatBranchRequest, user_id: str = Depends(get_current_user_id)):
    session = await chat_service.chat_dao.branch_session(session_id, request.before_message_id, user_id)
    if session is None:
        raise HTTPException(status_code=404, detail="用户消息不存在或无权访问")
    return session
...
@router.post("/chat", response_model=ChatResponse, summary="发送对话消息", dependencies=[Depends(enforce_request_quota)])
async def chat(
...
@router.post("/chat/stream", summary="发送对话消息（流式）", dependencies=[Depends(enforce_request_quota)])
async def chat_stream(
    ...
    try:
        await chat_service.prepare_chat(chat_request)
    except ConfigurationError as exc:
        raise HTTPException(status_code=400, detail="模型配置无效，请检查设置") from exc
    ...
    async def events():
        try:
            ...
            yield 'data: ' + json.dumps({"type": "error", "error": "对话生成失败，请重试"}, ensure_ascii=False) + '\n\n'
...
@router.post("/chat/web-search", response_model=ChatResponse, summary="联网搜索对话", dependencies=[Depends(enforce_request_quota)])
